...
export default function ContactInfoSection({
  ...
  contactInfoPromise: Promise<ContactInfoData>;
}) {
  const { primaryContactInfo, branchesContactInfo } = use(contactInfoPromise);

  // Separate main branch (type === 'Main') and normal branches
  const mainBranch = branchesContactInfo.find(
    (b) => (b.type || "").toLowerCase() === "main",
  );
  const branchAddresses = branchesContactInfo.filter(
    (b) => (b.type || "").toLowerCase() !== "main",
  );
  ...
  const allKeys: BranchKey[] = [
    ...(mainBranch ? ["main"] : []),
    ...groupedBranches.flatMap(([country, arr]) =>
      arr.map((_, i) => `${country}:${i}`),
    ),
  ];
  ...
  return (
    ...
    >
      ...
      <CollapsibleContent className="px-3 pb-3 pt-1 data-[state=closed]:animate-none">
        <div
          id={`panel-${heading}-${summary}`}
          className="mt-2 text-[13px] space-y-6"
        >
          {/* Address Table */}
          <div className="overflow-hidden rounded-md border bg-background">
            <Table>
              <TableBody>
                ...
                {branch.state && (
                  <VerticalRow label="State/Province" value={branch.state} />
                )}
                <VerticalRow label="Country" value={branch.country} />
                {isMain && (
                  <VerticalRow label="Zip Code" value={branch.zipCode} />
                )}
              </TableBody>
            </Table>
          </div>
                      ...
                      <VerticalRow
                        ...
                        value={
                          <>
                            ...
                            primaryContactInfo.isPersonPublished ? (
                              <Link
                            ...
                            {primaryContactInfo.contactPosition &&
                              ` (${primaryContactInfo.contactPosition})`}
                          </>
                        }
                      />
...
function VerticalRow({
  ...
  return (
    <TableRow className="*:border-border hover:bg-transparent [&>:not(:last-child)]:border-r">
      ...
      <TableCell className="py-2 align-top break-words leading-snug text-[13px]">
        {value}
      </TableCell>
    </TableRow>
  );
