...
export default function PreviousPositionsSection({ positions }: { positions: PersonPosition[] }) {
  const [showAll, setShowAll] = useState(false);
  const positionsToShow = showAll ? positions : positions.slice(0, PREVIOUS_POSITIONS_PREVIEW);

  return (
    <Item className="flex-col items-start p-0">
      <ItemTitle className="text-sm font-semibold">Previous Positions</ItemTitle>
      <ItemContent className="mt-3 w-full space-y-3">
        {positions.length > 0 ? (
          ...
          <p className="text-muted-foreground text-sm">No previous positions available.</p>
        )}
      </ItemContent>
    </Item>
  );
