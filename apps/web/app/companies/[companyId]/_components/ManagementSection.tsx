...
function orderManagement(list: CompanyExecutive[]) {
  ...
  return [...list].sort(
    (a, b) => weight(a.positionTitle) - weight(b.positionTitle),
  );
}
...
export default function ManagementSection({
  ...
  const managementToShow = showAllManagement
    ? orderedManagement
    : orderedManagement.slice(0, MANAGEMENT_PREVIEW);
                ...
                {managementToShow.map((managementPosition, idx) => (
                  <TableRow key={idx} className="text-[13px]">
                    <TableCell className="font-medium">
                      ...
                      managementPosition.isPersonPublished ? (
                        <Link
