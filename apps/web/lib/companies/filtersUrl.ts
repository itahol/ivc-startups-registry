...
const isSector = (sector: string): sector is SectorOption =>
  SECTOR_VALUES.includes(sector as SectorOption);

const isStage = (stage: string): stage is CompanyStageOption =>
  COMPANY_STAGE_VALUES.includes(stage as CompanyStageOption);

export function readCompanyFilters(
  ...
  const validMin =
    minNum !== undefined && !Number.isNaN(minNum) ? minNum : undefined;
  const validMax =
    maxNum !== undefined && !Number.isNaN(maxNum) ? maxNum : undefined;
...
export function encodeCompanyFilters(filters: CompanyFilters): URLSearchParams {
  ...
  if (filters.sectors?.length)
    sp.set("sectors", [...filters.sectors].sort().join(","));
  if (filters.stages?.length)
    sp.set("stages", [...filters.stages].sort().join(","));
  if (filters.yearEstablished?.min !== undefined)
    sp.set("ymin", String(filters.yearEstablished.min));
  if (filters.yearEstablished?.max !== undefined)
    sp.set("ymax", String(filters.yearEstablished.max));
